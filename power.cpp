...
Power::Power(int screenWidth, int screenHeight)
 {
    ...
    isActive = false;// Il ne tombe pas tout le temps
...
void Power::update(float deltaTime, Paddle* racket1,Paddle* racket2, float ballX, float ballY, float ballRadius, SDL_Renderer* renderer) {
    if (isActive) {
        ...
        // Réduire l'effet après le temps écoulé
        ...
        if (y + height >= WINDOW_HEIGHT || y <= 0) {
           ...
           if (effectActive) {
            ...
            if (effectDuration >= 5.0) {
                //std::cout << "Temps 2 restant effet: " << effectDuration << "s" << std::endl;
                //std::cout << "joueur effet: " << joueur << std::endl;
                if(joueur){
...
bool Power::checkCollision(float ballX, float ballY, float ballRadius) const {
    ...
    float closestX = (ballX < x) ? x : (ballX > x + width) ? x + width : ballX;
    float closestY = (ballY < y) ? y : (ballY > y + height) ? y + height : ballY;
